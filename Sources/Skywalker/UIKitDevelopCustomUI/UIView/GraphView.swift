...
extension GraphView {
    private enum Constants {
        static let cornerRadiusSize = CGSize.init(width: 8.0, height: 8.0) // 上下文裁剪圆角
...
public class GraphView: UIView {
    ...
    public override func draw(_ rect: CGRect) {
        ...
        let endPoint = CGPoint.init(x: 0, y: bounds.height)
        
        guard let gradient = CGGradient.init(colorsSpace: colorSpace, colors: colors as CFArray, locations: colorLocations) else { return }
        ...
        let graphPath = UIBezierPath.init()
        graphPath.lineWidth = 2.0
        graphPath.move(to: CGPoint.init(x: columXPoint(0), y: columYPoint(graphPoints[0])))
        for i in 1..<graphPoints.count {
            let nextPoint = CGPoint.init(x: columXPoint(i), y: columYPoint(graphPoints[i]))
